...
namespace TiltBrush
{
    ...
    public class EraserTool : StrokeModificationTool
    {
        ...
        override protected void SnapIntersectionObjectToController()
        {
            ...
            Vector3 scale = m_ToolTransform.localScale;

            //   m_ToolTransform.localScale = scale;

            if (App.VrSdk.controllerStyle == ControllerStyle.PicoHands)
            {
                //更新删除道具位置

                Transform rAttachPoint = BrushManager.Instance.biPoint;
                if (rAttachPoint)
                {
                    Vector3 pos = rAttachPoint.position + new Vector3(0f, 0f, 0f);
                    Quaternion rot = rAttachPoint.rotation;

                    m_ToolTransform.position = pos;
                    m_ToolTransform.rotation = rot;
                    m_CurrentSize = Mathf.Lerp(m_SizeRange.x, m_SizeRange.y, 0.5f);
                    m_ToolTransform.localScale = Vector3.one * m_CurrentSize * 1000f;
