...
namespace TiltBrush
{
    ...
    public partial class FreePaintTool : BaseTool
    {
        ...
        override public void UpdateTool()
        {
            ...
            if (BrushManager.Instance.brushhand.IsPinchByStrength() && !HandAndControllM_OpenBrush.Instance.InSelectUI)
            {
                m_PaintingActive = true;
                m_brushTriggerRatio = BrushManager.Instance.brushhand.PinchStrength();
            }
            else if (HandAndControllM_OpenBrush.Instance.InSelectUI)
            {
                m_PaintingActive = false;
                m_brushTriggerRatio = BrushManager.Instance.brushhand.PinchStrength();
            }
        ...
        void PositionPointer()
        {
            ...
            {
                PointerManager.m_Instance.MainPointer.transform.Find("Mesh").GetComponent<MeshRenderer>().enabled = true;

                PointerManager.m_Instance.SetPointerTransform(InputManager.ControllerName.Brush, pos, rot);
            }
            else
            {
                rAttachPoint = BrushManager.Instance.biPoint;
                if (rAttachPoint)
                {
                    pos = rAttachPoint.position + new Vector3(0f, 0f, 0f);
                    rot = rAttachPoint.rotation;
                    PointerManager.m_Instance.MainPointer.transform.Find("Mesh").GetComponent<MeshRenderer>().enabled = false;
                    PointerManager.m_Instance.SetPointerTransform(InputManager.ControllerName.Brush, pos, rot);
                }
