...
namespace TiltBrush
{
    ...
    public class SketchControlsScript : MonoBehaviour
    {
        ...
        private static bool sm_enableGrabHaptics = true;
        ...
        [SerializeField] private bool m_AtlasIconTextures;

        [SerializeField] private SaveIconTool m_SaveIconTool;
        [SerializeField] private DropCamWidget m_DropCam;
        [SerializeField] private string m_CreditsSketchFilename;
        [SerializeField] private string m_AshleysSketchFilename;
        [SerializeField] private float m_DefaultSketchLoadSpeed;
        [SerializeField] private GameObject m_TransformGizmoPrefab;

        [SerializeField] private GameObject m_RotationIconPrefab;
        [SerializeField] private float m_GazeMaxAngleFromFacing = 70.0f;
        [SerializeField] private float m_GazeMaxDistance = 10.0f;
        [SerializeField] private float m_GazeControllerPointingDistance;
        [SerializeField] private float m_GazePanelDectivationDelay = 0.25f;

        [SerializeField] private GameObject m_UIReticle;
        [SerializeField] private GameObject m_UIReticleMobile;
        [SerializeField] private GameObject m_UIReticleSixDofController;

        [SerializeField] private float m_DoubleTapWindow;
        [SerializeField] private float m_PushPullScale;
        [SerializeField] private RotationCursorScript m_RotationCursor;
        [SerializeField] private float m_RotationMaxAngle;

        [SerializeField] private float m_RotationScalar;
        [SerializeField] private float m_RotationRollScalar;
        [SerializeField] private float m_PanScalar;

        [SerializeField] private float m_AdjustToolSizeScalar;

        [SerializeField] private GameObject m_IRCChatPrefab;
        [SerializeField] private GameObject m_YouTubeChatPrefab;
        [SerializeField] private GameObject m_Decor;
        [SerializeField] private BaseTool.ToolType m_InitialTool = BaseTool.ToolType.SketchSurface;
        [SerializeField] private string m_ReleaseNotesURL;
        [SerializeField] private string m_HelpCenterURL;
        [SerializeField] private string m_ThirdPartyNoticesURL;
        [SerializeField] private string m_TosURL;
        [SerializeField] private string m_PrivacyURL;
        [SerializeField] private string m_QuestSideLoadingHowToURL;

        [Multiline]
        [SerializeField] private string m_ContributionPromoText;
        [SerializeField] private string m_ContributionURL;

        [SerializeField] private float m_WorldTransformMinScale = .1f;
        [SerializeField] private float m_WorldTransformMaxScale = 10.0f;

        [Header("Undo/Redo Hold")]
        [SerializeField] private float m_UndoRedoHold_DurationBeforeStart;
        [SerializeField] private float m_UndoRedoHold_RepeatInterval;

        [Header("Pin Cushion")]
        [SerializeField] private GameObject m_PinCushionPrefab;

        [Header("Grabbing and tossing")]
        [SerializeField] private float m_GrabWorldFadeSpeed = 8.0f;
        [SerializeField] private Color m_GrabWorldGridColor = new Color(0.0f, 1.0f, 1.0f, 0.2f);
        [SerializeField] private ControllerGrabVisuals m_ControllerGrabVisuals;
        [SerializeField] private float m_WidgetGpuIntersectionRadius;

        [Header("Saving")]
        [SerializeField] private int m_NumStrokesForSaveIcon = 50;
        ...
        public void InitGazePanels()
        {
            ...
            for (int i = 0; i < iNumGazePanels; ++i)
            {
                ...
                {
                    m_HitWithGaze = false,
                    m_HitWithController = false,
                    m_WithinView = false,
                    m_GazePosition = new Vector3()
                };
            }
        }
        ...
        private void Start()
        {
            m_TransformGizmo = Instantiate(m_TransformGizmoPrefab);
            ...
            m_RotationIcon = Instantiate(m_RotationIconPrefab);
            ...
            GameObject pinCushionObj = Instantiate(m_PinCushionPrefab);
                ...
                if (bWidgetGrabOK && !m_GrabBrush.grabbingWorld)
                {
                    ...
                    else
                    {
                        ...
                        if (m_SketchSurfacePanel.ActiveTool.LockPointerToSketchSurface())
                        {
                            ...
                            if (App.VrSdk.controllerStyle != ControllerStyle.PicoHands)
                            {
        ...
        private void UpdateStandardInput()
        {
            ...
            {
                MultiCamTool camTool = SketchSurfacePanel.m_Instance.ActiveTool as MultiCamTool;
                ...
                {
                    SymmetryMode cur = PointerManager.m_Instance.CurrentSymmetryMode;
                    SymmetryMode next = (cur == SymmetryMode.None) ? SymmetryMode.SinglePlane
        ...
        {
            ControllerInfo controller = InputManager.Controllers[(int)m_GrabWidgetOneHandInfo.m_Name];
            ...
            else
            {
                ...
                {
                    ControllerInfo info = InputManager.Controllers[(int)m_GrabWidgetOneHandInfo.m_Name];
                    TrTransform controllerXf = Coords.AsGlobal[info.Transform];
                    TrTransform newWidgetXf = controllerXf * m_GrabWidgetOneHandInfo.m_BaseWidgetXf_LS;
        ...
        private void RequestWidgetIntersection(List<GrabWidgetData> candidates,
                                       InputManager.ControllerName controllerName)
        {
            ...
            if (requestGpuIntersection)
            {
                ...
                {
                    resultList = new List<GpuIntersector.ModelResult>()
                };
        ...
        private GrabWidgetData GetBestWidget(List<GrabWidgetData> candidates,
                                     Queue<GpuIntersectionResult> resultQueue)
        {
            ...
            for (int i = 0; i < candidates.Count; ++i)
            {
                GrabWidgetData candidate = candidates[i];
                if (!candidate.m_NearController)
                {
                    continue;
                }
                ...
                if (candidate.m_WidgetScript is MediaWidget
                    && candidate.m_WidgetScript.Canvas != App.Scene.ActiveCanvas)
                {
                    continue;
                }
        ...
        private void UpdateGrabWidgetHoldHistory(InputManager.ControllerName name)
        {
            ...
            ControllerInfo info = InputManager.Controllers[(int)name];
        ...
        private void OnBrushSetToDefault()
        {
            ...
            PointerManager.m_Instance.SetAllPointersBrushSize01(0.01f);
        ...
        private IEnumerator<Null> ExportListAndQuit()
        {
            ...
            foreach (string filePattern in App.Config.m_FilePatternsToExport)
            {
                ...
                string[] tiltFiles = Directory.GetFiles(directory, filename);
                filesToExport.AddRange(tiltFiles);
                // Also look at .tilt files which have been unzipped into directory format
                IEnumerable<string> tiltDirs = Directory.GetDirectories(directory, filename)
                    .Where(n => n.EndsWith(".tilt"));
                filesToExport.AddRange(tiltDirs);
            }

            using (IEnumerator<Null> coroutine = LoadAndExportList(filesToExport))
            {
        ...
        private IEnumerator<Null> LoadAndExportList(List<string> filenames)
        {
            ...
            {
                using (IEnumerator<Null> coroutine = LoadAndExport(filename))
                {
                    while (coroutine.MoveNext())
                    {
                        yield return coroutine.Current;
        ...
        private IEnumerator<Null> LoadAndExportAll()
        {
            ...
            for (int i = 0; i < SketchCatalog.m_Instance.GetSet(SketchSetType.User).NumSketches; ++i)
            {
                ...
                using (IEnumerator<Null> coroutine = LoadAndExport(rInfo.FullPath))
                {
                    while (coroutine.MoveNext())
                    {
                        yield return coroutine.Current;
        ...
        {
            using (SceneSettings.RequestInstantSceneSwitch unused = new SceneSettings.RequestInstantSceneSwitch())
            {
                IssueGlobalCommand(
        ...
        {
            foreach (Null val in LoadTiltFile(filename))
            {
                yield return val;
            }
            using (IEnumerator<Null> coroutine = ExportCoroutine())
            {
                while (coroutine.MoveNext())
                {
                    yield return coroutine.Current;
        ...
        public TrTransform GenerateBestGuessSaveIconTransform(int itemsToEnumerate = 0)
        {
            ...
            Stroke[] lastFewStrokes = SketchMemoryScript.AllStrokes().Skip(startIndex).ToArray();

            Bounds bounds;
            if (lastFewStrokes.Length > 0)
            {
                bounds = new Bounds(lastFewStrokes.First().m_ControlPoints.First().m_Pos, Vector3.zero);
                foreach (Stroke stroke in lastFewStrokes.Skip(1))
                {
            ...
            else
            {
                ...
                ModelWidget[] models = m_WidgetManager.ModelWidgets.ToArray();
                                       ...
                                       int iParam2 = -1, string sParam = null)
        {
            switch (rEnum)
            {
                ...
                case GlobalCommands.MergeBrushStrokes:
                    {
                        ...
                        int index = iParam1;
                        SketchSetType sketchSetType = (SketchSetType)iParam2;
                ...
                case GlobalCommands.IRC:
                    ...
                    {
                        GameObject widgetobject = Instantiate(m_IRCChatPrefab);
                ...
                case GlobalCommands.YouTubeChat:
                    ...
                    {
                        GameObject widgetobject = Instantiate(m_YouTubeChatPrefab);
                        widgetobject.transform.parent = App.Instance.m_RoomTransform;
                ...
                case GlobalCommands.RenameLayer:
                    {
                        CanvasScript layer = App.Scene.GetCanvasByLayerIndex(iParam1);
                ...
                case GlobalCommands.LoginToGenericCloud:
                    {
                        OAuth2Identity ident = App.GetIdentity((Cloud)iParam1);
                ...
                case GlobalCommands.UploadToGenericCloud:
                    {
                        ...
                        OAuth2Identity ident = App.GetIdentity(cloud);
                        if (!ident.LoggedIn)
                        {
                            ident.LoginAsync();
                            break;
                ...
                case GlobalCommands.ViewLastUpload:
                    ...
                    {
                        string url = VrAssetService.m_Instance.LastUploadCompleteUrl;
                        App.OpenURL(url);
                ...
                case GlobalCommands.LoadWaitOnDownload:
                    {
                        ...
                        {
                            BasePanel sketchBook = m_PanelManager.GetSketchBookPanel();
                            SketchSet googleSketchSet = SketchCatalog.m_Instance.GetSet(SketchSetType.Drive);
                            if (sketchBook != null
                ...
                case GlobalCommands.DeleteCameraPath:
                    {
                        TypedWidgetData<CameraPathWidget> cameraPath = m_WidgetManager.GetCurrentCameraPath();
        ...
        {
            DiskSceneFileInfo fileInfo = new DiskSceneFileInfo(path);
